...
def run_feedback_turn(
    ...
    """
    # Detect feedback
    is_feedback = _detect_feedback(message, client)
    if not is_feedback:
        return {"type": "ignore"}
    
    # Extract outcome and target signals
    outcome = _extract_outcome(message, client)
    target_signals = _extract_target_signals(message, client)
    ...
    routine_id = resolve_routine_for_feedback(
...
def _detect_feedback(message: str, client) -> bool:
    ...
    try:
        ...
        return result.get("is_feedback", False)
    except Exception:
        return False


def _extract_outcome(message: str, client) -> Dict[str, str]:
...
def _extract_target_signals(message: str, client) -> Dict[str, Any]:
